...
function compactFacts(facts) {
  return compactObject({
    ...
    twitterUrl: facts.twitterUrl || "",
    leetcodeUrl: facts.leetcodeUrl || "",
    ...
    freelanceExperience: facts.freelanceExperience || "",
...
function buildSystemMessage() {
  return [
    "You are an expert job application assistant. You fill out job application forms accurately using only the provided candidate profile and answer bank.",
    "CRITICAL RULES:",
    "1. Never invent facts. If evidence is missing from the profile or answer bank, return answer null.",
    "2. For text fields (name, email, phone, etc.), return the exact value from the profile. Be concise and direct.",
    "3. For select/radio fields, ALWAYS pick one of the provided option values. Match the closest option to the profile data.",
    "   - For 'how did you hear about us' or similar, pick 'Online job board' or 'Job posting' if available, otherwise the most generic option.",
    "   - For 'years of experience' dropdowns, map the profile's experience to the closest matching option.",
    "   - For 'work type' or 'employment type', pick 'Full-time' if available.",
    "   - For 'degree' or 'education level', pick the option closest to 'B.Tech' or 'Bachelor's degree'.",
    "   - For boolean yes/no fields, return the appropriate boolean.",
    "4. For textarea fields (cover letter, additional info, why interested, etc.), write 2-4 professional sentences using profile facts. Use action + impact format.",
    "5. For 'cover letter' or 'additional information' fields, write a brief professional paragraph highlighting: self-taught background, MeowFi internship achievements (40% RPC reduction, monorepo architecture), PrepZone production deployment, and technical stack proficiency.",
    "6. Return strict JSON that matches the schema.",
    "7. Be confident. Use values from the profile directly. Do not hedge or add disclaimers.",
  ].join(" ");
}
